...
def train_val_data_process():
    train_data = FashionMNIST(root = 'D:/code/MyModel/GoogLeNet/data',
                          train = True,
...
def train_model_process(model,train_dataloader,val_dataloader,num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for step,(b_x,b_y) in enumerate(train_dataloader):
            b_x = b_x.to(device) #b_x:128*1*28*28(以fashionMNIST为例)
            b_y = b_y.to(device) #b_y:128*1
            model.train()
        ...
        with torch.no_grad():
            for step,(b_x,b_y) in enumerate(val_dataloader):
                b_x = b_x.to(device) #b_x:128*1*28*28
                b_y = b_y.to(device) #b_y:128*1
                model.eval()
...
if __name__ == "__main__":
    ...
    train_process = train_model_process(model,train_dataloader,val_dataloader,num_epochs=40)
